...
//        endcase
        if (keycode[7:0] == 8'H0D || keycode[15:8] == 8'H0D || keycode[23:16] == 8'H0D || keycode[31:24] == 8'H0D  ) //left 
        begin
           Ball_X_next = BallX - 5;
        end 
        ...
        if( (BallX + BallS <= 90) && (BallY+BallS >= 270) && (BallY  <= 340)  ) //left wall second floor     //flag
        begin 
            Ball_X_next = BallX + 1;
        end  
...
//            end
            ...
            unique case (state)
                ...
                begin
                    ...
                    if(({ared, agreen, ablue} != 12'hF4F) || {ared, agreen, ablue} != 12'h0FF 
                    || ({ared, agreen, ablue} != 12'h1D0) || {ared, agreen, ablue} != 12'hB3B) 
                     begin
//                      Pink = BallY;
                        state <= IN_AIR;
                        end
                     if({ared, agreen, ablue} == 12'h0FF || ({ared, agreen, ablue} == 12'h1D0)) // if  i.e green || blue then dead
                     begin
                        dead <= 1;
                     end
...
//                      Pink = BallY;
                     ...
                     if({ared, agreen, ablue} == 12'h0FF || ({ared, agreen, ablue} == 12'h1D0)) // if dead i.e green || blue
                     begin
                        dead <= 1;
                        state <= ON_GROUND;
                        BallY <= BallY;
